...
def calculate_dataset_stats(dataset_path):
    ...
    img_paths = sorted([p for p in images_dir.iterdir() if p.suffix.lower() in ['.jpg', '.jpeg', '.png']])
    
    print(f"Calculating mean and std from {len(img_paths)} training images...")
    ...
    print(f"Calculated mean: [{mean[0]:.3f}, {mean[1]:.3f}, {mean[2]:.3f}]")
    print(f"Calculated std: [{std[0]:.3f}, {std[1]:.3f}, {std[2]:.3f}]")
    
    return mean.tolist(), std.tolist()

calculated_mean, calculated_std = calculate_dataset_stats(DATASET_ROOT)
...
def train_model():
    ...
    print(f"Loss: Hybrid Loss (Focal + CIoU/DIoU + Tweak)")
    ...
    train_dataset = ElephantDataset(str(train_images), str(train_labels), IMG_SIZE, train_transforms)
    val_dataset = ElephantDataset(str(val_images), str(val_labels), IMG_SIZE, val_transforms)
...
def find_test_images():
    ...
    if test_dir.exists():
        test_images.extend(glob.glob(str(test_dir / "*.jpg")))
        test_images.extend(glob.glob(str(test_dir / "*.jpeg")))
        test_images.extend(glob.glob(str(test_dir / "*.png")))
    
    if not test_images:
        val_dir = Path(DATASET_ROOT) / 'valid' / 'images'
        if val_dir.exists():
            val_images = list(val_dir.glob("*.jpg")) + list(val_dir.glob("*.jpeg")) + list(val_dir.glob("*.png"))
            test_images = [str(img) for img in val_images[:3]]
...
def inference(image_path, checkpoint_path, conf_threshold=0.25):
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        ...
        if 'dataset_mean' in checkpoint and 'dataset_std' in checkpoint:
            inference_mean = checkpoint['dataset_mean']
            inference_std = checkpoint['dataset_std']
        else:
            inference_mean = calculated_mean
            inference_std = calculated_std
    else:
    ...
    processed_image = (processed_image - np.array(inference_mean)) / np.array(inference_std)
...
def run_complete_pipeline():
    print("STARTING COMPLETE YOLO ELEPHANT DETECTION PIPELINE")
    print("=" * 80)
    ...
    if not test_images:
        print("No test images found. Please add images to test/images/ or valid/images/")
        return
